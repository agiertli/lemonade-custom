...
MODEL_NAME = os.getenv("MODEL_NAME", "gravitee-io/distilbert-multilingual-toxicity-classifier")
...
app = FastAPI(title="Multilingual HAP Detector", lifespan=lifespan)
...
def detect_toxicity(text: str) -> List[ContentAnalysisResponse]:
    ...
    try:
        ...
        logger.error(f"Error detecting toxicity for '{text}': {e}")
        return []
...
async def analyze_contents(request: ContentAnalysisHttpRequest):
    tasks = [
        asyncio.to_thread(detect_toxicity, content)
